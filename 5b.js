...
function updateCustomTriggerEditorClick(tx, ty) {
	if (!lcPropertiesMode || tx < 0 || ty < 0 || tx >= levelWidth || ty >= levelHeight) return false;
	let tile = myLevel[1][ty][tx];
...
function updateCustomTriggersRuntime() {
	...
	for (let i = 0; i < lcTriggerLinks.length; i++) {
		let link = lcTriggerLinks[i];
		let trigKey = `${link.id}:${link.triggerX},${link.triggerY}`;
		if (firedCustomTriggers[trigKey]) continue;
		let run = false;
...
function draw() {
	...
	switch (menuScreen) {
		...
		case 5:
			...
			if (mouseIsDown && !_keysDown[32]) {
				if (selectedTab == 2 && lcPropertiesMode && mouseOnGrid() && !pmouseIsDown) {
					if (updateCustomTriggerEditorClick(x, y)) updateLCtiles();
